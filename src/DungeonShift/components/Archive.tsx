...
export default function Archive({ community, mine, loaded, notice, onBack, onBuild, onRaid }: { community: WallEntry[]; mine: DungeonConfig[]; loaded: boolean; notice?: string; onBack: () => void; onBuild: () => void; onRaid: (dungeon: DungeonConfig, author?: DungeonAuthor) => void }) {
  ...
  const entries = useMemo(() => {
    const cloudByDungeon = new Map(community.map((entry) => [entry.dungeon.id, entry]));
    const mineEntries: WallEntry[] = mine.map((dungeon) => ({ ...cloudByDungeon.get(dungeon.id), userId: 'self', userName: t('you'), dungeon, plays: cloudByDungeon.get(dungeon.id)?.plays ?? 0 }));
    if (scope === 'mine') return mineEntries;
    const mineIds = new Set(mine.map((dungeon) => dungeon.id));
    const others = community.filter((entry) => !mineIds.has(entry.dungeon.id));
    ...
    return unique.length ? unique.slice(0, 24) : loaded ? [{ userId: 'sample', userName: t('sampleKeeper'), dungeon: SAMPLE_DUNGEON, plays: 0 }] : [];
  }, [community, loaded, mine, scope]);
  ...
  return <section className="ds-archive" aria-labelledby="archive-title">
    ...
    <div className="ds-archive__list">
      {!loaded && scope === 'community' && entries.length === 0 && <div className="ds-empty">{t('loading')}</div>}
